...
// B2: Duyệt qua toàn bộ sản phẩm (đối tượng dữ liệu có trong products)
...
for (let i in products) {
  ...
  let productCart = `
    <div class="product-card">
        ...
        <button class="add-to-cart-btn" data-id="${products[i].id}">Thêm vào giỏ hàng</button>
    </div>
    `;
...
// console.log(addBtnList); (Bước kiểm tra sau khi gán)
...
for(let btn of addBtnList) {
    btn.onclick = function() {
        ...
        // console.log(products[index]); (kiểm tra xem lấy thành công vị trí chưa)
        // {id, name, price, image, number}
        // Kiểm tra xem sản phẩm có trong giỏ hàng hay chưa ??
        // Nếu chưa -> Tạo ra 1 đối tượng {id, name, price, image, number}
        // với number = 1 -> và thêm đối tượng đó vào trong giỏ hàng
        // Tăng trường number lên 1 đơn vị
...
//     if(cart.length === 0) {
//         // Hiển thị giỏ hàng trống trong card
...
//         // B4.3: Tìm ra được vị trí của sản phẩm đó -> Tiến hành cập nhật lại số lượng của sản phẩm đó trong mảng cart
...
// // B4.4: Phản ánh sự thay đổi đó ở trên trình duyệt = cách gọi lại hàm renderCart
// renderCart();
    ...
    if (cart.length === 0) {  
        ...
        for (let btn of increaseList) {  
            btn.onclick = function() {  
                ...
                if (index !== -1) { // Kiểm tra sản phẩm tồn tại trong giỏ hàng  
                    cart[index].number++; // Tăng số lượng  
        ...
        for (let btn of decreaseList) {  
            btn.onclick = function() {  
                let id = btn.getAttribute("data-id");  
                let index = cart.findIndex((element) => element.id === +id);  
                if (index !== -1 && cart[index].number > 0) { // Kiểm tra sản phẩm tồn tại và số lượng lớn hơn 0  
                    cart[index].number--; // Giảm số lượng  
                    if (cart[index].number === 0) {  
                        cart.splice(index, 1); // Xóa sản phẩm khỏi giỏ hàng nếu số lượng = 0  
                    }  
                    renderCart(); // Cập nhật lại giỏ hàng  
                }  
            }  
        ...
        // Gán sự kiện cho các nút xóa sản phẩm  
